...
def test_parse_tableset():
    ...
    assert len(tables) == 2, "Number of tables expected: %d, found: %d" % (
        2,
        len(tables),
    )
...
def test_parse_votable_error_response(stored_responses):
    ...
    assert (
        message == "Cannot parse query 'select top 5 * from gg.gaia_source' for"
...
def test_job_error_message(stored_responses):
    ...
    message = utils.Job.parse_xml(stored_responses["async_query"].text)["message"]
    assert message is None

    message = utils.Job.parse_xml(stored_responses["async_wrong_query"].text)["message"]
    assert (
        message == "Cannot parse query 'select top 5 * from gg.gaia_source' for job"
